...
item for the formal exit criterion)

**Done**:
- Decided every flag of `ng new` upfront (`npx` install, zoneless, SCSS,
  no SSR, name `pecunia-frontend`).
- Upgraded the local Node runtime to 24.16.0 Krypton LTS via nvm (Angular
  CLI 22 requires ≥ 22.22.3 / 24.15 / 26) and pinned it via
  `apps/frontend/.nvmrc`.
- Scaffolded the Angular 22 app under `apps/frontend/`. Validated: `ng
  build --configuration=development` succeeds in 1.9 s, `ng serve` boots in
  552 ms, `curl http://localhost:4200/` returns HTTP 200.
- Investigated two suspected anomalies in the generated config and
  dismissed both: (a) zoneless is the v21+ default and needs no explicit
  `provideZonelessChangeDetection()` call, (b) TypeScript strict mode is
  effectively active despite `"strict": true` not appearing in the
  generated `tsconfig.json` — verified by a deliberate `tsc` probe
  (`TS7006` + `TS2322` fire).
- Replaced the generic Angular CLI README with a Pecunia-specific
  minimal version (prerequisites, commands, pointers to root docs).
- Anchored the root `.gitignore` rule `.vscode/` → `/.vscode/` so
  `apps/frontend/.vscode/` could be whitelisted by the local `.gitignore`.
- Two commits on the branch (not yet pushed): `chore: scope .vscode/ rule
  to repo root in .gitignore` and `feat(frontend): scaffold Angular 22
  application` (25 files, +9494 lines).

**Learned**:
- Angular CLI 22 ships several new defaults that shape Pecunia: Vitest 4
  + jsdom 28 as the test runner replacing Karma/Jasmine (with
  `@angular/build:unit-test`), file naming without the `.component`
  suffix (`app.ts` / `App` class — effective since v20), `public/` as the
  static assets folder, `@angular/build` as the unified
  build/serve/unit-test builder (esbuild + Vite under the hood).
- Zoneless became the v21+ default: removing `zone.js` from the deps is
  sufficient; the provider call is only needed on v20 or earlier.
- TypeScript 6 + Angular CLI 22 inherit strict-mode behavior without
  spelling `"strict": true` in the generated `tsconfig.json`. The probe
  technique (untyped param + null-to-string assignment compiled with
  `tsc --noEmit`) is a cheap way to verify strict is active.
- `.gitignore` precedence in Git: a parent directory excluded by a
  bare pattern (`.vscode/`) cannot be re-included from a child
  `.gitignore` via `!` exceptions; the parent rule must be anchored
  (`/.vscode/`) instead.
- Operational note for the Bash tool: each invocation is a fresh shell,
  so `nvm use` must be re-applied each time. The `.nvmrc` pin is the
  long-term safety net; in scripted multi-command shells, exporting
  `PATH="$HOME/.nvm/versions/node/<v>/bin:$PATH"` and `hash -r` is the
  reliable workaround.
- `.vscode/mcp.json` is a new convention: declares an MCP server (here
  `@angular/cli mcp`) that VS Code AI integrations can call for
  Angular-aware completions.

**Next**:
- Push the branch (2 commits ahead of origin).
- `ng add @angular/material@22` with a chosen M3 palette.
- `proxy.conf.json` for `ng serve` → backend `:8080` on `/oauth2`,
  `/login`, `/logout`, future `/api/**`.
- OpenAPI codegen pipeline (`@openapitools/openapi-generator-cli` + npm
  script, mirroring the Maven plugin on the backend side).
- Auth flow shell: public landing → `/oauth2/authorization/pecunia` link,
  protected `/me` route consuming the generated client. Closes the
  Block 1 exit criterion.
- CSRF wiring with `provideHttpClient(withXsrfConfiguration(...))` against
  the backend `csrf().spa()` configuration.
- PWA + ESLint + Prettier integration.

**Notes**:
- `launch.json` references `http://localhost:9876/debug.html` for `ng
  test` — Karma's historical port. Vitest's debug URL differs; to
  re-check the first time we debug a unit test from VS Code.
- `.editorconfig` generated by Angular CLI to be reconciled with the
  backend's when the monorepo-wide formatting policy is firmed up.

See [detailed recap](docs/session-recaps/2026-06/2026-06-16-session-10.md).
